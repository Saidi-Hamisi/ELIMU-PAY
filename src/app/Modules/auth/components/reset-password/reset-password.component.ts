      ...
      this.authService.forgotPassword(this.formData.value).subscribe(
        (res: { statusCode: number; message: any }) => {
          if (res && res.statusCode === 200 && res.message) {
            ...
            this.notificationAPI.alertWarning('Password reset failed. Please try again.');
          }
          this.loading = false;
        },
        (error: any) => {
          this.notificationAPI.alertWarning('An error occurred while resetting password. Please try again later.');
          this.loading = false;
        }
      );
    } else {
